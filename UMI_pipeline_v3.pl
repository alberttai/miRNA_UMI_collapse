...
while (<IN>){
	...
	if ( length ($umiplus) >= 12 && $line[1] != -1 && $umi !~ m/N/ && $read !~ m/N/ ){
		if ( $read_len >= $min_len) {
			...
			if ( $umi =~ m/^A/ ){
				$a_array[$a_array_count][0] = $line[0]; #unique ID
				$a_array[$a_array_count][1] = $line[4]; #sequence
				$a_array[$a_array_count][2] = $line[8]; #quality score
				$a_array[$a_array_count][3] = $umi;
				$a_array[$a_array_count][4] = $line[4]."_".$umi;
				$a_array_count++;
			} elsif ($umi =~ m/^C/ ){
				...
				$c_array[$c_array_count][4] = $line[4]."_".$umi;
				$c_array_count++;
			} elsif ($umi =~ m/^G/ ){
				...
				$g_array[$g_array_count][4] = $line[4]."_".$umi;
				$g_array_count++;
			} else {
				...
				$t_array[$t_array_count][4] = $line[4]."_".$umi;
				$t_array_count++;
			}
...
my @sorted_a_array = sort { $a->[4] cmp $b->[4]} @a_array;
print "Sorting sorted UMI started with C\n";
my @sorted_c_array = sort { $a->[4] cmp $b->[4]} @c_array;
print "Sorting sorted UMI started with G\n";
my @sorted_g_array = sort { $a->[4] cmp $b->[4]} @g_array;
print "Sorting sorted UMI started with T\n";
my @sorted_t_array = sort { $a->[4] cmp $b->[4]} @t_array;
...
for (my $i = 1; $i < $sorted_read_array_count; $i++){
	...
	my $curr_sequmi = $sorted_read_array[$i][4];
	my $prev_sequmi = $sorted_read_array[$i-1][4];

	#print "$dedup_read_array_count\t$curr_umi\t$curr_seq\t$curr_seq_length\n\n";

	if ( $curr_sequmi eq $prev_sequmi) {
